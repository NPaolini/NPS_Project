...
*/
//----------------------------------
bool manCookie::isValid(const char* c, int8_t& currUser)
{
  currUser = -1;
  if(!*c)
    return false;
...
uint8_t manCookie::add(const char* c, int8_t idUser)
{
  ...
  for(uint8_t i = 0; i < SIZE_A(Cookie); ++i) {
    ...
    // non puo' succedere, ma ...
    if(Cookie[i].idUser == idUser) {
      pos = i;
      break;
      }
...
//----------------------------------
void manCookie::rem(int8_t user)
{
  for(uint8_t i = 0; i < SIZE_A(Cookie); ++i) {
    if(user == Cookie[i].idUser) {
      Cookie[i].c[0] = 0;
      Cookie[i].startTime = 0;
      }
    }
