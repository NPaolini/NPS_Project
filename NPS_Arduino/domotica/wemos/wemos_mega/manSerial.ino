...
void GS_getIP()
{
  ...
  // [ip],[dns],[gateway],[port]#
  g_Buff[0] = 0;
  Serial3.print(F("cmd=getip:#"));
  int len = Serial3.readBytesUntil('#', g_Buff, sizeof(g_Buff));
  g_Buff[len] = 0;
  Serial.print(g_Buff);
  Serial.println(F("#"));
}
...
{
  Serial.print(F("smtp:"));
  Serial.println(ISetup.iEmail.smtp);
  Serial.print(F("port:"));
  Serial.println(ISetup.iEmail.port);
  Serial.print(F("from:"));
  Serial.println(ISetup.iEmail.from);
  Serial.print(F("to:"));
  Serial.println(ISetup.iEmail.to);
  Serial.print(F("user:"));
  Serial.println(ISetup.iEmail.user);
  Serial.print(F("psw:"));
  Serial.println(ISetup.iEmail.psw);
}
//-------------------------------------------------------------------
void GS_getEmail()
{
  Serial.print(F("r=getemail:s,"));
  Serial.print(ISetup.iEmail.smtp);
  Serial.print(F(",p,"));
  Serial.print(ISetup.iEmail.port);
  Serial.print(F(",f,"));
  Serial.print(ISetup.iEmail.from);
  Serial.print(F(",t,"));
  Serial.print(ISetup.iEmail.to);
  Serial.print(F(",u,"));
  Serial.print(ISetup.iEmail.user);
  Serial.print(F(",w,"));
  Serial.print(ISetup.iEmail.psw);
  Serial.println(F("#"));
}
//-------------------------------------------------------------------
...
void GS_setEmail()
{
  ...
  do {
    ...
    switch(*cmd) {
      case 's':
        GS_copyEmailData(ISetup.iEmail.smtp, g_Buff);
        break;
      case 'p':
        ISetup.iEmail.port = atoi(g_Buff);
        break;
      case 'f':
        GS_copyEmailData(ISetup.iEmail.from, g_Buff);
        break;
      case 't':
        GS_copyEmailData(ISetup.iEmail.to, g_Buff);
        break;
      case 'u':
        GS_copyEmailData(ISetup.iEmail.user, g_Buff);
        break;
      case 'w':
        GS_copyEmailData(ISetup.iEmail.psw, g_Buff);
        break;
      }
    } while(true);
  saveSetup();
...
void GS_setWIFI()
{
  uint8_t len = Serial.readBytesUntil('#', g_Buff, sizeof(g_Buff));
  g_Buff[len] = 0;
  Serial3.print(F("cmd=wifi:"));
  Serial3.print(g_Buff);
  Serial3.print(F(",#"));
  char resp[] = "r=wifi:";
  g_Buff[0] = 0;
  if(Serial3.find(resp, strlen(resp))) {
    len = Serial3.readBytesUntil('#', g_Buff, sizeof(g_Buff));
    g_Buff[len] = 0;
    }
