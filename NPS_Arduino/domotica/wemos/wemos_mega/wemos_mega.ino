...
//--------------------------------------------------
pin_t lastPinSPI = -1;
static pin_t allpin[] = { PIN_SD_CARD, RFID_SS_PIN, TFT_CS
#ifdef USE_SPI_SHIFT_REG
  , PIN_SHIFT_REG_LATCH
#endif
  };
//--------------------------------------------------
...
//--------------------------------------------------
void enableSD() { enableGen(PIN_SD_CARD); }
...
#else
pin_t mplexSel1[] = { MPLEX_1_S1, MPLEX_1_S2, MPLEX_1_S3, MPLEX_1_S4, MPLEX_1_EN };
mplex MPlex1(MPLEX_1_Z, mplexSel1);
pin_t mplexSel2[] = { MPLEX_2_S1, MPLEX_2_S2, MPLEX_2_S3, MPLEX_2_S4, MPLEX_2_EN };
mplex MPlex2(MPLEX_2_Z, mplexSel2);
#endif
...
bool G_digRead(pin_t pin)
{
  ...
  else if(OFFS_MPX_4 > pin)
    return MPlex3.getStatus(REM_MPX_3(pin));
  return false;
#endif
}
//---------------------------------
...
void setup()
{
  ...
  enableSD();
  bool success = SD.begin(PIN_SD_CARD);
  DEBUG_SETUP(Serial.print("SD Loaded: "); Serial.println(success ? F("Ok") : F("Failed")))
...
void updateCronoWorking(bool force)
{
  ...
  C_T.setItems((byte*)g_Buff, DIM_CRONO, 2);
}
//-------------------------------------------------------------------
...
void loop()
{
  ...
  if(needSendEmail > 0) {
    ...
    for(uint8_t i = 0; i < num; ++i) {
      if(Alarm.getItemStatus(i, true)) {
        const char* ena = Alarm.isExcluted(i) ? "" : "(*)";
        sprintf(p, " %s%s[%d],", ena, alname[i], i + 1);
        p += strlen(p);
        }
      }
    *--p = 0;
...
//-------------------------------------------------------------------
// spostando la gestione cookie nel esp8622, l'idUser viene
// passato come ultimo parametro:
// cmd=x:a,idUser#
// cmd=x:e,ix,idUser#
// cmd=x:x,from,to,on_off,idUser#
// int8_t idUser = getLastParam(cmd);

uint8_t getLastParam(const char* cmd)
{
...
//------------------------------
void fillRain(char* buff, int len)
{
  sprintf(buff, "1|b|%d|", currRain);
}
//-------------------------------------------------------------------
...
#ifndef USE_HW_TIME
//------------------------------
void myRtc::run(bool reload)
{
  if(reload)
    Serial3.print(F("cmd=synctime:1#"));
  else
    Serial3.print(F("cmd=synctime:0#"));
  for(int i = 0; i < 50; ++i) {
